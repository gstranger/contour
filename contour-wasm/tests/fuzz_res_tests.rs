...
fn fuzz_strict_methods_no_abort() {
    ...
    for _ in 0..500u32 {
        ...
        let res = match op {
            ...
            10 => {
                // polyline invalid array
                let arr = Float32Array::from(&[0.0f32, 1.0, 2.0][..]);
                g.add_polyline_edge_res(a, b, &arr)
            }
            11 => {
                // region invalid key
            ...
            13 => {
                ...
                let ids = Uint32Array::from(&[a, 0xFFFF_FFFF][..]);
                g.translate_nodes_res(&ids, 1.0, 2.0)
            }
            _ => unreachable!(),
        };
        // No aborts and no state mutation on error paths
        if !is_ok(&res) {
            assert_eq!(g.geom_version(), ver_before);
        }
    }

    // Valid parity checks (a few calls)
    let v = g.add_node_res(10.0, 20.0);
    assert!(is_ok(&v));
    let id = Reflect::get(
        &Reflect::get(&v, &JsValue::from_str("value")).unwrap(),
        &JsValue::from_str("value"),
    )
    .ok();
    let _ = id;
    let p = g.pick_res(50.0, 0.0, 10.0);
    assert!(is_ok(&p));
}
