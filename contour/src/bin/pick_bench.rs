...
fn build_grid_graph(edges_target: usize) -> Graph {
    ...
    let w = 120usize;
    let h = 120usize; // up to 14k edges available
    let mut nodes = Vec::with_capacity(w * h);
    for j in 0..h {
        for i in 0..w {
            nodes.push(g.add_node(i as f32 * 8.0, j as f32 * 6.0));
        }
    }
...
fn main() {
    ...
    for a in &args[1..] {
        if let Some(val) = a.strip_prefix("--edges=") {
            if let Ok(v) = val.parse() {
                edges = v;
            }
        } else if let Some(val) = a.strip_prefix("--picks=") {
            if let Ok(v) = val.parse() {
                picks = v;
            }
        } else if let Some(val) = a.strip_prefix("--tol=") {
            if let Ok(v) = val.parse() {
                tol = v;
            }
        } else if let Some(val) = a.strip_prefix("--assert-ms=") {
            if let Ok(v) = val.parse() {
                assert_ms = Some(v);
            }
        }
    ...
    println!(
        "edges={} picks={} tol={} hits={} total_ms={:.3} median_ms={:.4} p90_ms={:.4} p99_ms={:.4}",
        edges, picks, tol, hits, dur_all, med, p90, p99
    );
    if let Some(th) = assert_ms {
        if med > th {
            eprintln!("FAIL: median {:.4} ms > threshold {:.3} ms", med, th);
            std::process::exit(1);
        }
    }
}
