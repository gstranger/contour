...
#[inline]
fn within_eps(x: f64, eps: f64) -> bool {
    x.abs() <= eps
}

#[inline]
fn clamp01f64(x: f64) -> f64 {
    if x < 0.0 {
        0.0
    } else if x > 1.0 {
        1.0
...
fn collinear_overlap(
    ...
    if within_eps(len_ab, eps) {
        ...
        let u = if within_eps(len_cd, eps) {
            ...
            (if dxab >= dyab { x - cx } else { y - cy }) / len_cd
        };
    ...
    let u0 = if within_eps(len_cd, eps) {
        ...
        (pa1 + lo * len_ab - pc1) / len_cd
    };
    let u1 = if within_eps(len_cd, eps) {
        0.0
    } else {
        (pa1 + hi * len_ab - pc1) / len_cd
    };
...
pub fn intersect_segments(
    ...
    let inter1 = (o1 > 0.0 && o2 < 0.0)
        || (o1 < 0.0 && o2 > 0.0)
        || within_eps(o1, eps)
        || within_eps(o2, eps);
    let inter2 = (o3 > 0.0 && o4 < 0.0)
        || (o3 < 0.0 && o4 > 0.0)
        || within_eps(o3, eps)
        || within_eps(o4, eps);
    if !(inter1 && inter2) {
    ...
    let is_touch = within_eps(t, eps)
        || within_eps(1.0 - t, eps)
        || within_eps(u, eps)
        || within_eps(1.0 - u, eps);

    if is_touch {
        SegIntersection::Touch {
            t: clamp01f64(t),
            u: clamp01f64(u),
            x,
            y,
...
mod tests {
    ...
    fn endpoint_touch() {
        ...
        match r {
            SegIntersection::Touch { t, u, x, y } => {
                assert!((x - 1.0).abs() < 1e-9 && (y - 0.0).abs() < 1e-9);
    ...
    fn collinear_overlap() {
        ...
        match r {
            SegIntersection::CollinearOverlap { t0, t1, .. } => {
                assert!(t0 >= 0.33 && t1 <= 0.67);
            }
