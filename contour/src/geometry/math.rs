...
pub fn cubic_distance_sq(
    ...
    if approx_eq(x0, x1, EPS_POS)
        && approx_eq(x1, x2, EPS_POS)
        && approx_eq(x2, x3, EPS_POS)
        && approx_eq(y0, y1, EPS_POS)
        && approx_eq(y1, y2, EPS_POS)
        && approx_eq(y2, y3, EPS_POS)
    {
        let dx = px - x0;
        let dy = py - y0;
