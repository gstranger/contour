...
pub fn from_json_impl(g: &mut Graph, v: Value) -> bool {
    ...
    let parsed:Result<DocDe,_>=serde_json::from_value(v); if let Ok(doc)=parsed {
        // Caps: sizes
        if doc.nodes.len() > limits::MAX_NODES || doc.edges.len() > limits::MAX_EDGES { return false; }
        // Validate nodes
        for n in &doc.nodes { if !limits::in_coord_bounds(n.x) || !limits::in_coord_bounds(n.y) { return false; } }
            ...
            if let Some(kind) = &e.kind {
                match kind {
                    ...
                    EdgeDeKind::Cubic{ ha, hb, .. } => {
                        ...
                        if !limits::in_coord_bounds(ha.x) || !limits::in_coord_bounds(ha.y) { return false; }
                        if !limits::in_coord_bounds(hb.x) || !limits::in_coord_bounds(hb.y) { return false; }
                    }
                    EdgeDeKind::Polyline{ points } => {
                        if points.len() > limits::MAX_POLYLINE_POINTS_PER_EDGE { return false; }
                        poly_total += points.len(); if poly_total > limits::MAX_POLYLINE_POINTS_TOTAL { return false; }
                        for p in points { if !limits::in_coord_bounds(p.x) || !limits::in_coord_bounds(p.y) { return false; } }
                    }
                }
            }
            if let Some(w)=e.width { if !limits::in_width_bounds(w) { return false; } }
            if let Some(c)=e.stroke { let _ = (c.r,c.g,c.b,c.a); }
        }
        ...
        for n in doc.nodes { if !limits::in_coord_bounds(n.x) || !limits::in_coord_bounds(n.y) { return false; } g.nodes[n.id as usize]=Some(crate::model::Node{ x:n.x, y:n.y }); }
        for e in doc.edges {
            ...
            let width = e.width.unwrap_or(2.0); if !limits::in_width_bounds(width) { return false; }
            g.edges[e.id as usize]=Some(crate::model::Edge{ a:e.a,b:e.b,kind,stroke:e.stroke,stroke_width: width });
        }
...
pub fn from_json_impl_strict(g: &mut Graph, v: Value) -> Result<bool, (&'static str, String)> {
    ...
    if doc.nodes.len() > limits::MAX_NODES { return Err(("caps_exceeded", format!("nodes>{}", limits::MAX_NODES))); }
    if doc.edges.len() > limits::MAX_EDGES { return Err(("caps_exceeded", format!("edges>{}", limits::MAX_EDGES))); }
    for n in &doc.nodes { if !limits::in_coord_bounds(n.x) || !limits::in_coord_bounds(n.y) { return Err(("out_of_bounds", "node coordinate".into())); } }
    let mut poly_total: usize = 0;
    for e in &doc.edges {
        if e.a==e.b { return Err(("invalid_structure", "edge endpoints equal".into())); }
        if let Some(kind) = &e.kind {
            match kind {
                EdgeDeKind::Line => {}
                EdgeDeKind::Cubic{ ha, hb, .. } => {
                    if !limits::in_coord_bounds(ha.x) || !limits::in_coord_bounds(ha.y) { return Err(("out_of_bounds", "ha".into())); }
                    if !limits::in_coord_bounds(hb.x) || !limits::in_coord_bounds(hb.y) { return Err(("out_of_bounds", "hb".into())); }
                }
                EdgeDeKind::Polyline{ points } => {
                    if points.len() > limits::MAX_POLYLINE_POINTS_PER_EDGE { return Err(("caps_exceeded", format!("polyline_points_per_edge>{}", limits::MAX_POLYLINE_POINTS_PER_EDGE))); }
                    poly_total += points.len(); if poly_total > limits::MAX_POLYLINE_POINTS_TOTAL { return Err(("caps_exceeded", format!("polyline_points_total>{}", limits::MAX_POLYLINE_POINTS_TOTAL))); }
                    for p in points { if !limits::in_coord_bounds(p.x) || !limits::in_coord_bounds(p.y) { return Err(("out_of_bounds", "polyline point".into())); } }
                }
            }
        }
        if let Some(w)=e.width { if !limits::in_width_bounds(w) { return Err(("out_of_bounds", "width".into())); } }
    }
    let max_node=doc.nodes.iter().map(|n| n.id).max().unwrap_or(0);
    let max_edge=doc.edges.iter().map(|e| e.id).max().unwrap_or(0);
    g.nodes=vec![None; (max_node as usize)+1]; g.edges=vec![None; (max_edge as usize)+1]; g.fills.clear();
    for n in doc.nodes { if !limits::in_coord_bounds(n.x) || !limits::in_coord_bounds(n.y) { return Err(("out_of_bounds", "node coordinate".into())); } g.nodes[n.id as usize]=Some(crate::model::Node{ x:n.x, y:n.y }); }
    for e in doc.edges {
        let a_ok = g.nodes.get(e.a as usize).and_then(|n| *n).is_some();
        let b_ok = g.nodes.get(e.b as usize).and_then(|n| *n).is_some();
        if !a_ok || !b_ok { continue; }
        ...
        let width=e.width.unwrap_or(2.0); if !limits::in_width_bounds(width) { return Err(("out_of_bounds", "width".into())); }
        g.edges[e.id as usize]=Some(crate::model::Edge{ a:e.a,b:e.b,kind,stroke:e.stroke,stroke_width: width });
    }
    if let Some(fills)=doc.fills { for f in fills { g.fills.insert(f.key, FillState{filled:f.filled,color:f.color}); } }
