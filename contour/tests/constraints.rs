...
fn mirrored_bend_preserves_invariants() {
    ...
    let (ax, ay) = g.get_node(a).unwrap();
    let (bx, by) = g.get_node(b).unwrap();
    let ha1 = (h1[0] - ax, h1[1] - ay);
    let hb1 = (h1[2] - bx, h1[3] - by);
    let la1 = (ha1.0 * ha1.0 + ha1.1 * ha1.1).sqrt();
    let lb1 = (hb1.0 * hb1.0 + hb1.1 * hb1.1).sqrt();
    ...
    let ha2 = (h2[0] - ax, h2[1] - ay);
    let hb2 = (h2[2] - bx, h2[3] - by);
    let la2 = (ha2.0 * ha2.0 + ha2.1 * ha2.1).sqrt();
    let lb2 = (hb2.0 * hb2.0 + hb2.1 * hb2.1).sqrt();
