...
fn determinism_and_persistence() {
    ...
    let mut g = Graph::new();
    let w = 8usize;
    let h = 8usize; // keep test runtime bounded in CI
    let mut nodes = Vec::new();
    for j in 0..h {
        for i in 0..w {
            nodes.push(g.add_node(i as f32 * 8.0, j as f32 * 8.0));
        }
    }
    ...
    for step in 0..800 {
        if step % 20 == 0 {
            ...
            let dx = (lcg(&mut seed) - 0.5) * 0.6;
            let dy = (lcg(&mut seed) - 0.5) * 0.6;
            ...
            let tx = lcg(&mut seed) * 160.0;
            let ty = lcg(&mut seed) * 160.0;
            let _ = g.bend_edge_to(eid, t, tx, ty, 1.0);
        }
        // Trigger recompute + remap
        if step % 40 == 0 {
            // measure infrequently to reduce runtime
            let regs = g.get_regions();
            let filled = regs
                .iter()
                .filter(|v| v.get("filled").and_then(|x| x.as_bool()).unwrap_or(false))
                .count();
            let total_now = regs.len().max(1);
            let rate = (filled as f64) / (total_now as f64);
            accumulated_ratio += rate;
            samples += 1;
            if log_progress {
                eprintln!(
    ...
    let avg = if samples > 0 {
        accumulated_ratio / (samples as f64)
    } else {
        1.0
    };
...
fn quick_persistence_smoke() {
    ...
    for j in 0..h {
        for i in 0..w {
            nodes.push(g.add_node(i as f32 * 10.0, j as f32 * 10.0));
        }
    }
    for j in 0..h {
        for i in 0..(w - 1) {
    ...
    if !set0.is_empty() {
        let rate = (inter as f64) / (set0.len() as f64);
        assert!(rate >= 0.98, "quick persistence {:.3} below expected", rate);
    }
}

// Release-only: 1k edits persistence >= 99.5%
// Disabled by default unless feature "long-persist" is enabled to avoid
// long-running CI or local runs. Enable with:
//   cargo test --release -p contour --features long-persist --test regions_persist -- release_persistence_1k
#[cfg(all(not(debug_assertions), feature = "long-persist"))]
#[test]
fn release_persistence_1k() {
    ...
    for v in regs0.iter() {
        ...
        for i in (0..pts.len()).step_by(2) {
            let j = (i + 2) % pts.len();
            let xi = pts[i].as_f64().unwrap() as f32;
            let yi = pts[i + 1].as_f64().unwrap() as f32;
            let xj = pts[j].as_f64().unwrap() as f32;
            let yj = pts[j + 1].as_f64().unwrap() as f32;
        ...
        cx /= (6.0 * a);
        cy /= (6.0 * a);
        let qx = (cx * 10.0).round() as i32;
        let qy = (cy * 10.0).round() as i32;
        let fill = ((qx + qy) & 1) == 0;
        ...
        regs.iter()
            .map(|v| {
                ...
                if a.abs() < 1e-2 {
                    ...
                    let qx = (cx * 10.0).round() as i32;
                    let qy = (cy * 10.0).round() as i32;
                    let filled = v.get("filled").and_then(|x| x.as_bool()).unwrap_or(false);
                    (qx, qy, filled)
                }
            })
            .collect()
    ...
    for step in 0..1000 {
        if step % 25 == 0 {
            ...
            let dx = ((seed >> 8) as f32 / (u32::MAX as f32) - 0.5) * 0.6;
            let dy = ((seed >> 16) as f32 / (u32::MAX as f32) - 0.5) * 0.6;
            ...
            let t = (((seed >> 24) as f32) / (u32::MAX as f32))
                .min(1.0)
                .max(0.0);
            let tx = ((seed >> 28) as f32 % 160.0);
            let ty = ((seed >> 32) as f32 % 160.0);
            let _ = g.bend_edge_to(eid, t, tx, ty, 1.0);
        }
        let regs = g.get_regions();
        ...
        for v in regs.iter() {
            ...
            let qy = (cy * 10.0).round() as i32;
            let filled = v.get("filled").and_then(|x| x.as_bool()).unwrap_or(false);
            curr.push((qx, qy, filled));
        }
        // For each new face, find nearest prev by qcentroid and compare filled
        for (qx, qy, fill) in curr.iter() {
            let mut best_d2 = i64::MAX;
            let mut best_f = false;
            for (pqx, pqy, pf) in prev.iter() {
                let dx = *qx as i64 - *pqx as i64;
                let dy = *qy as i64 - *pqy as i64;
                let d2 = dx * dx + dy * dy;
                if d2 < best_d2 {
                    best_d2 = d2;
                    best_f = *pf;
    ...
    if total > 0 {
        ...
        assert!(
            rate >= 0.995,
            "release persistence {:.5} below target",
            rate
        );
    }
}
