...
pub(crate) fn to_svg_paths_impl(g: &Graph) -> JsValue {
    let mut paths: Vec<String> = Vec::new();
    for e in g.edges.iter() {
        if let Some(edge) = e {
            let a = match g.nodes.get(edge.a as usize).and_then(|n| *n) { Some(n) => n, None => continue };
            let b = match g.nodes.get(edge.b as usize).and_then(|n| *n) { Some(n) => n, None => continue };
            match &edge.kind {
                EdgeKind::Line => {
...
pub(crate) fn add_svg_path_impl(g: &mut Graph, d: &str) -> u32 {
    parse_and_add(g, d, None)
}

pub(crate) fn add_svg_path_with_style_impl(g: &mut Graph, d: &str, rgba: Option<(u8,u8,u8,u8,f32)>) -> u32 {
    parse_and_add(g, d, rgba)
}

fn parse_and_add(g: &mut Graph, d: &str, rgba: Option<(u8,u8,u8,u8,f32)>) -> u32 {
    let mut i = 0usize;
        ...
        match cmd {
            b'M'|b'm' => {
                ...
                loop {
                    ...
                    if let Some(eid) = g.add_edge(a_id, b_id) { if let Some((r,gc,b,aa,w)) = rgba { g.set_edge_style(eid, r, gc, b, aa, w); } edges_added += 1; }
                    cur = (nx, ny);
                }
            }
            b'L'|b'l' => {
            ...
            b'C'|b'c' => {
                loop {
                    ...
                    if let Some(eid) = g.add_edge(a_id, b_id) {
                        ...
                        if let Some((r,gc,b,aa,w)) = rgba { g.set_edge_style(eid, r, gc, b, aa, w); }
                        edges_added += 1;
                    }
                    cur = (x, y);
                    skip_ws(bytes, &mut i);
            ...
            b'Z'|b'z' => {
                ...
                if a_id != b_id { if let Some(eid) = g.add_edge(a_id, b_id) { if let Some((r,gc,b,aa,w)) = rgba { g.set_edge_style(eid, r, gc, b, aa, w); } edges_added += 1; } }
                cur = start_sub;
            }
            _ => { /* ignore */ }
        }
    }
    if edges_added > 0 { g.geom_ver = g.geom_ver.wrapping_add(1); }
    edges_added
}
